...
fn build_response(request: &Request, static_root: &str) -> Response {
    ...
    if request.method != "GET" {
        ...
        if request.path.len() as i32 == 1 {
            add_file_to_response(&"/index.html".to_string(), &mut response, static_root)
        } else {
    ...
    match contents {
        ...
        Err(e) => {
            response.status = match e.kind() {
                ...
                ErrorKind::PermissionDenied => http::StatusCode::FORBIDDEN,
